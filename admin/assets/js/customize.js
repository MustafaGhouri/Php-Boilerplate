...
$(document).on('click','.selecteddelte',function(){
    let idies = [];
        $('.jneiflow.active').each(function(){
          let val = $(this).find('.jneiflow-td').attr('data-id');
           idies.unshift(val);
        })
     swal({
      title: 'Are you sure?',
      ...
      if (result.value) {
          ...
          $.ajax({
            url: siteurl+'admin/include/delete.php?page=bulk',
            type: "POST",
            data: {idies:idies},
            dataType: "JSON",
            success: function (result) {
...
jQuery(document).on("submit","#deleteNews", function (e) {
    //   alert();
      e.preventDefault();
      $(".btn-sbmit").attr("disabled", "disabled");
      var page = $("#deletel_page").val();
      var news_id = $("#dell_page_id").val();
      $.ajax({
        ...
        success: function (result) {
          ...
          if (result['res'] == "success") { 
            ...
            $("#deleteNewsModal").modal("hide");
              var rowTr  = '#'+news_id+'news';
             
              $(rowTr).parent().parent().fadeOut(700, function() {
                    $(this).remove();
                });
